...
# rodar antes pq sempre precisa reiniciar sessão
...
def process_dir_adv(INPUT_FOLDER: str, OUTPUT_FOLDER: str):
    ...
    for img_path in image_paths:
        ...
        try:
            ...
            prediction = torch.argmax(probs, dim=1).item()  # integer class
            confidence = probs[0, prediction].item()        # float in [0,1]
            print("predicted class: ",classes[prediction])
            print("confidence: ", confidence)

            # ART classifier prediction
            predictions = classifier.predict(x)  # shape: (1, 1000)

            # Top-1 index and class
            class_idx = int(np.argmax(predictions, axis=1)[0])
            class_name = classes[class_idx]

            # Softmax confidence
            probs = softmax_activation(predictions)
            confidence = float(probs[0, class_idx] * 100)

            print(f"Predicted class: {class_name}")
            print(f"Confidence: {confidence:.2f}%")
            #----------------------------------------------------

            # Read image for Grad-CAM (OpenCV uses BGR → convert to RGB)
            rgb_img = cv2.imread(img_path, cv2.IMREAD_COLOR)[:, :, ::-1]
            rgb_img = np.float32(rgb_img) / 255.0

            # Pre-process for Grad-CAM
...
input_image = Image.open('input_images/tabby-224.jpg')
...
def custom_preprocess_image(img: Image.Image):
    # Convert PIL → numpy (HWC)
    arr = np.array(img).astype(np.float32) / 255.0  # (224,224,3)

    # HWC → CHW (correct order!)
...
# --------------------------------------------------------------------
# Save adversarial image generated by ART
# --------------------------------------------------------------------
def save_adversarial_image(x_adv, save_path):
...
# --------------------------------------------------------------------
# Model wrapper: normalizes inside the model (ART provides unnormalized input)
# --------------------------------------------------------------------
...
# --------------------------------------------------------------------
# Main function: generate adversarial samples for a folder
# --------------------------------------------------------------------
def generate_adversarial_folder(input_folder, output_folder,
                                eps=0.01, eps_step=1.0, max_iter=20, device="cuda"):
    ...
    # Load clean model
    base_model = resnet50(weights='DEFAULT').to(device)
    base_model.eval()

    # Wrap model so ART receives raw [0,255] inputs
    mean = [0.485, 0.456, 0.406]
    std  = [0.229, 0.224, 0.225]
    model = NormalizedModel(base_model, mean, std).to(device).eval()

    # Loss does not matter much for PGD; using CE is fine
    ...
    classifier = PyTorchClassifier(
        model=model,
        loss=criterion,
        optimizer=None,      # no training, so no optimizer needed
        input_shape=(3, 224, 224),
        nb_classes=1000,
    ...
    for path in image_paths:
        ...
        try:
            ...
            # Run PGD attack
            x_adv = pgd.generate(x)

            # Save adversarial sample
            save_adversarial_image(x_adv, out_path)
...
"""#### resultados saudaveis"""

process_dir_adv('input_images', 'result_images')

"""#### resultados adversariais"""

process_dir_adv('adv_images', 'adv_results')
