...
def run_gradcam(filename:str, targets:List[ClassifierOutputTargets],
                ...
                output_folder_prefix:str, grad_cam:GradCAM, prediction:int):
    ...
    # Save grad-cam img
    if(targets is None) : # label will be predicted class
        classes_labels = classes[prediction]
    ...
    save_path =  os.path.join(save_folder, f"{classes_labels};{filename}")
    cv2.imwrite(save_path, gradcam_img[:, :, ::-1])
    
def run_counterfactual_gradcam(filename:str, targets:List[ClassifierOutputTargets],
                input_tensor:torch.Tensor, rgb_img:np.ndarray, 
                output_folder_prefix:str, counterfactual_grad_cam:CounterfactualGradCAM, prediction:int):
    # Generate heatmp from Grad-CAM
    counterfactual_grad_cam_vis = counterfactual_grad_cam(input_tensor=input_tensor, targets=targets)[0, :]

    # Overlay heatmap in image
    gradcam_img = show_cam_on_image(rgb_img, counterfactual_grad_cam_vis, use_rgb=True)
    ...
    if(targets is None): # label will be predicted class
        ...
        classes_labels = classes[targets[0].categories]
    save_folder = os.path.join(output_folder_prefix, "counterfactual_explanations")
    os.makedirs(save_folder, exist_ok=True)
    save_path =  os.path.join(save_folder, f"{classes_labels};{filename}")
    cv2.imwrite(save_path, gradcam_img[:, :, ::-1])
